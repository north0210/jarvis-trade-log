...
describe("clampToCoverage（幅保持クランプ）", () => {
  ...
  it("coverageStart 省略時は始端クランプなし（従来動作を維持）", () => {
    const r = clampToCoverage("2021-07-13", "2026-07-13", "2026-07-10");
    expect(r.to).toBe("2026-07-10");
    expect(r.from < "2021-07-13").toBe(true); // 幅保持で開始日より前へずれる（従来動作）
  });
  it("始端クランプ: 終端寄せで from が購読開始日より前へずれたら開始日へ是正（400の主因）", () => {
    // Light 5年窓 [2021-07-13, 2026-07-10]。5年要求 to=今日=2026-07-13 は範囲外。
    // 幅保持だと from=2021-07-10（開始日より前）→ 400。始端クランプで 2021-07-13 へ。
    const r = clampToCoverage("2021-07-13", "2026-07-13", "2026-07-10", "2021-07-13");
    expect(r.clamped).toBe(true);
    expect(r.to).toBe("2026-07-10");
    expect(r.from).toBe("2021-07-13"); // 開始日へクランプ（幅は縮む）
  });
  it("始端クランプ: to がカバレッジ内でも from が開始日より前なら是正", () => {
    const r = clampToCoverage("2020-01-01", "2026-01-01", "2026-04-12", "2021-07-13");
    expect(r.clamped).toBe(true);
    expect(r.from).toBe("2021-07-13");
    expect(r.to).toBe("2026-01-01"); // 終端は範囲内なので不変
  });
});
